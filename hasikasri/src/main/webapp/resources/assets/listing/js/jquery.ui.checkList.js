(function($) {
	$.widget("ui.checkList", {
		...
		_create : function() {
			...
			var chkAll = $('<input/>').attr('type', 'checkbox').addClass(
			...
			var txtfilter = $('<input/>').attr('type', 'text').attr(
					'placeholder', '  Search').addClass('txtFilter').keyup(
					function() {
						self._filter($(this).val());
					});
			// toolbar.append(chkAll); // uncomment this to enable check All
			// check box control
			toolbar.append($('<div/>').addClass('filterbox').text('').append(
					txtfilter));

			// generate list table object
			o.objTable = $('<div/>').addClass('table refiners_scroll');
			// o.objTable = $('<div/>').addClass('refiners_scroll').append(
			// '<table/>');
		...
		_addItem : function(listItem) {
			...
			var chk = $('<input/>').attr('type', 'checkbox').attr('id', itemId)
					.addClass('chk').attr('data-text', listItem.text).attr(
			...
			o.objTable.find('.chk').each(
					function() {
						if ($(this).attr('checked')) {
							...
							$(this).parent().addClass('highlight').siblings()
									.addClass('highlight');
		...
		_filter : function(filter) {
			...
			o.objTable.find('.chk').each(
					function() {
						if ($(this).attr('data-text').toLowerCase().indexOf(
